...
def draw_window_buttons():
    ...
    mouse = pygame.mouse.get_pos()
    for rect, draw_func in [(close_rect, lambda r: pygame.draw.line(screen, RED, (r.x+5, r.y+5), (r.x+BTN_SIZE-5, r.y+BTN_SIZE-5), 2)),
                            (fs_rect, lambda r: pygame.draw.rect(screen, BLUE, (r.x+5, r.y+5, BTN_SIZE-10, BTN_SIZE-10), 2)),
                            (min_rect, lambda r: pygame.draw.line(screen, BLUE, (r.x+5, r.y+BTN_SIZE-5), (r.x+BTN_SIZE-5, r.y+BTN_SIZE-5), 2))]:
...
# --- Main Game Function with Difficulty ---
def main_game():
    ...
    while running and STATE == "GAME":
        ...
        if len(pipes) == 0 or pipes[-1]['x'] < WIDTH - 300:
            upper_height = random.randint(50, HEIGHT - current_pipe_gap - GROUND_HEIGHT - 50)
        ...
        for pipe in pipes:
            ...
            lower_rect = pygame.Rect(pipe['x'], pipe['upper'] + current_pipe_gap, PIPE_WIDTH, HEIGHT - pipe['upper'] - GROUND_HEIGHT)
        ...
        for pipe in pipes:
            if not pipe['passed'] and pipe['x'] + PIPE_WIDTH < 100:
                ...
                if score % 5 == 0:
                    ...
                    current_pipe_gap = max(120, current_pipe_gap - 10)
                    current_gravity += 0.05
        ...
        angle = max(-25, min(90, bird_velocity * 3))
        ...
        for pipe in pipes:
            ...
            pygame.draw.rect(screen, GREEN, (pipe['x'], pipe['upper'] + current_pipe_gap, PIPE_WIDTH, HEIGHT - pipe['upper'] - GROUND_HEIGHT))

        ground_scroll -= current_pipe_speed
